...
Ngày tạo: 2025-05-12
Phiên bản: 1.1 - Fixed duplicate key and update issues
*/
...
BEGIN
    ...
    DELETE FROM MODEL_REGISTRY.dbo.MODEL_TYPE WHERE TYPE_CODE = 'TEST_UNIT';
    ...
    BEGIN TRY
        ...
        VALUES ('TEST_UNIT', 'Test Model Type Unit', 'Type created for unit testing');
...
BEGIN
    ...
    -- Cleanup any existing test data first
    DELETE FROM MODEL_REGISTRY.dbo.MODEL_REGISTRY WHERE MODEL_NAME = 'TEST_MODEL_UNIT';
    
    -- Tạo một bản ghi kiểm thử trong MODEL_REGISTRY nếu TYPE_ID đã được tạo
    ...
    ELSE
    BEGIN
        PRINT 'LỖI: Không thể tạo bản ghi kiểm thử trong MODEL_REGISTRY. Lỗi: Violation of UNIQUE KEY constraint';
        SET @ERROR_COUNT = @ERROR_COUNT + 1;
    END
END
...
BEGIN TRY
    DECLARE @INVALID_TYPE_ID INT = (SELECT ISNULL(MAX(TYPE_ID), 0) + 999 FROM MODEL_REGISTRY.dbo.MODEL_TYPE);
    ...
    -- Cleanup nếu insert thành công (không nên xảy ra)
    DELETE FROM MODEL_REGISTRY.dbo.MODEL_REGISTRY WHERE MODEL_NAME = 'INVALID_MODEL_TEST';
END TRY
BEGIN CATCH
...
BEGIN
    BEGIN TRY
        ...
        VALUES (
            @NEW_TYPE_ID,
            'TEST_MODEL_UNIT', -- Trùng với bản ghi đã tạo
...
ELSE
BEGIN
    PRINT 'Bỏ qua kiểm thử cập nhật do không có dữ liệu test.';
END
...
BEGIN
    ...
    ELSE
    BEGIN
        PRINT 'Bỏ qua kiểm thử trigger audit do không có dữ liệu test.';
    END
END
ELSE
BEGIN
